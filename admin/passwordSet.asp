  ...
  if session("adminUsername")="" then
	...
	if Request("submit") <> "" then
	  ...
	  if newPassword <> newPasswordAgain then
		...
		if errMessage = "" then
		  sqlString = "select * from [admin] where adminUsername='" & session("adminUsername") & "'"
		  set adminInfoRs = Server.CreateObject("ADODB.RecordSet")
		  adminInfoRs.Open sqlString,conn,1,3
		  '判断输入的原来密码是否正确
		  if adminInfoRs("adminPassword") <> md5(oldPassword) then
		    Response.Write "<script>alert('你输入的旧密码不正确!');</script>"
			else
			  adminInfoRs("adminPassword") = md5(newPassword)
			  adminInfoRs.Update
