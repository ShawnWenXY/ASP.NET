...
<%
  dim  sqlString,courseNumber,courseName,specialFieldNumber,specialFieldName,termId,termName
  '如果管理员还没有登陆
  if session("adminUsername")="" then
	...
	'取得该课程的专业编号和学期编号等信息
	sqlString = "select * from [publicCourseInfo] where courseNumber='" & courseNumber & "'"
	...
	if not publicCourseInfoRs.EOF then
	  ...
	  termId = publicCourseInfoRs("termId")
	  ...
	  if Request("teacherNumber") = "" then
	    ...
	    if Request("MorningOne") = "1" then
		  ...
		  if conflictMessage <> "" then
		    ...
		    '下面开始将新的上课信息加入到数据库中
		    sqlString = "select * from [publicCourseTeach]"
		    set publicCourseTeachRs = Server.CreateObject("ADODB.RecordSet")
		    publicCourseTeachRs.Open sqlString,conn,1,3
		    publicCourseTeachRs.AddNew
		    publicCourseTeachRs("courseNumber") = courseNumber
		    publicCourseTeachRs("termId") = CInt(termId)
			  publicCourseTeachRs("specialFieldNumber") = specialFieldNumber
		    publicCourseTeachRs("teacherNumber") = Request("teacherNumber")
		    publicCourseTeachRs("teachClassRoom") = Request("teachClassRoom")
		    publicCourseTeachRs("teachDay") = CInt(Request("teachDay"))
		    publicCourseTeachRs("MorningOne") = MorningOne
		    publicCourseTeachRs("MorningTwo") = MorningTwo
		    publicCourseTeachRs("MorningThree") = MorningThree
		    publicCourseTeachRs("MorningFour") = MorningFour
		    publicCourseTeachRs("MorningFive") = MorningFive
		    publicCourseTeachRs("AfternoonOne") = AfternoonOne
		    publicCourseTeachRs("AfternoonTwo") = AfternoonTwo
		    publicCourseTeachRs("AfternoonThree") = AfternoonThree
		    publicCourseTeachRs("AfternoonFour") = AfternoonFour
		    publicCourseTeachRs("EveningOne") = EveningOne
		    publicCourseTeachRs("EveningTwo") = EveningTwo
		    publicCourseTeachRs("EveningThree") = EveningThree
		    publicCourseTeachRs.Update
   ...
   <input type="hidden" name=courseNumber value=<%=Request("courseNumber")%>>
	 <table width=700 border=0 cellpadding=0 cellspacing=0 align="center">
	     ...
	     <td style="height: 14px" colspan=2 align="center">
		      <img src="../images/ADD.gif" width=14px height=14px>排课信息管理--&gt;选修课排课信息添加
			 </td>
	   </tr>
		 <tr>
		   <td width=100 align="right">课程信息:</td>
			 <td><% Response.Write specialFieldName & " 专业 " & termName & " " & courseName%></td>
		 </tr>
		 <tr>
        ...
        <td height="30" colspan="2" align="center">
		      <input name="submit"  type="submit" value=" 确认添加 "> &nbsp;
				 <input type="button" value="返回" onClick="javascript:location.href='publicCourseTeachMakeSecond.asp?courseNumber=<%=courseNumber%>'">
		    </td>
      </tr>
	 </table>
 </form>
